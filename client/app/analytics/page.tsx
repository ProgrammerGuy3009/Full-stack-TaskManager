  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (response.success) {
        ...
        setAnalytics({
          totalTasks: response.stats?.totalTasks ?? 0,
          completedTasks: response.stats?.completedTasks ?? 0,
          pendingTasks: response.stats?.pendingTasks ?? 0,
          highPriorityTasks: response.stats?.highPriorityTasks ?? 0,
          overdueTasks: response.stats?.overdueTasks ?? 0,
          completionRate: response.stats?.completionRate ?? 0,
          tasksByStatus: {
            todo: response.stats?.tasksByStatus?.todo ?? 0,
            inProgress: response.stats?.tasksByStatus?.inProgress ?? 0,
            completed: response.stats?.tasksByStatus?.completed ?? 0,
          },
          tasksByPriority: {
            low: response.stats?.tasksByPriority?.low ?? 0,
            medium: response.stats?.tasksByPriority?.medium ?? 0,
            high: response.stats?.tasksByPriority?.high ?? 0,
          },
          weeklyProgress,
        });
      }
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByStatus?.todo ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByStatus?.todo ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByStatus?.inProgress ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByStatus?.inProgress ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByStatus?.completed ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByStatus?.completed ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByPriority?.high ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByPriority?.high ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByPriority?.medium ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByPriority?.medium ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontWeight: '600' }}>{analytics.tasksByPriority?.low ?? 0}</span>
                    </div>
                    <div style={{ width: '100%', height: '8px', backgroundColor: '#f3f4f6', borderRadius: '4px' }}>
                      <div
                        style={{
                          width: `${analytics.totalTasks > 0 ? ((analytics.tasksByPriority?.low ?? 0) / analytics.totalTasks) * 100 : 0}%`,
                          height: '100%',
                          backgroundColor: '#16a34a',
                          borderRadius: '4px'
                        }}
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'end', gap: '20px', height: '200px', padding: '0 20px' }}>
                {analytics.weeklyProgress.map((day, index) => {
                  ...
                  const completedHeight = maxValue ? (day.completed / maxValue) * 150 : 0;
                  const createdHeight = maxValue ? (day.created / maxValue) * 150 : 0;
                  return (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                ...
                <div style={{ padding: '15px', backgroundColor: '#fef3c7', borderRadius: '8px', border: '1px solid #fed7aa' }}>
                  ...
                  <p style={{ color: '#92400e', margin: 0, fontSize: '14px' }}>
                    {analytics.tasksByPriority?.high ?? 0 > analytics.totalTasks * 0.4
